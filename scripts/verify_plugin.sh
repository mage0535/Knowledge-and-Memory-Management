...
echo "── 记忆体系统 ──"
curl -sf http://localhost:8787/api/stats &>/dev/null && echo "  ✓ gbrain (8787)" || { echo "  - gbrain (未安装/未运行, 可选)"; }
...
echo ""
echo "── 插件目录 ──"
[ -d "${HOME}/.hermes/knowledge/notes" ] && echo "  ✓ 笔记目录" || { echo "  ✗ 笔记目录缺失"; errors=$((errors+1)); }
[ -d "${HOME}/.hermes/knowledge-plugin" ] && echo "  ✓ Python 模块" || { echo "  - Python 模块 (未安装)"; }
